...
**Command line.** `pyligent-agents` (`setup`, `steps`, `doctor`, `reconcile`,
`validation-pack`, `new`, `graph`, `runs`, `trace`) and `evidence-check`.
`reconcile` compares signed agreements against a collateral system's stored
terms and writes nothing — see [RECONCILE.md](RECONCILE.md). `setup` reports what
